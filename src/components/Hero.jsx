...
const Hero = () => {
  ...
  return (
    <>
    <div className="grid grid-rows-16 grid-cols-8 overflow-hidden gap-2 w-full bg-[#846c65] relative">
      {images.map((image) => {
        return (
          <div
            key={image.id}
            className="elem col-span-1 row-span-1 "
            style={{ "--r": Math.floor(Math.random() * 15 + 1), "--c": Math.floor(Math.random() * 7 + 1 ) }}
          >
            <img
        ...
        <p className="text-center text-gray-300 text-2xl tracking-widest">
